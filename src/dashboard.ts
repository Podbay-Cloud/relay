...
<script>
const app={data:null,selectedPod:null,mode:'all',outcomes:new Set(),site:'',stopped:false,tab:'overview',activityView:'events',range:24,rangeLabel:'Last 24 hours',shown:50,openEvents:new Set()};const PAGE_SIZE=50;const OUTCOME_GROUPS=[['ok','OK'],['refused','Refused'],['blocked','Blocked'],['error','Error']];const OUTCOME_PRED={ok:e=>e.outcome==='ok',refused:e=>e.outcome==='site-refused',blocked:e=>e.outcome==='owner-blocked'||e.outcome==='safety-blocked',error:e=>e.outcome==='network-error'||e.outcome==='rate-limited'};
...
document.addEventListener('click',async e=>{const tab=e.target.closest('[data-tab]');if(tab){activateTab(tab.dataset.tab);return}const jump=e.target.closest('[data-tab-jump]');if(jump){activateTab(jump.dataset.tabJump);return}const activityView=e.target.closest('[data-activity-view]');if(activityView){activateActivityView(activityView.dataset.activityView);return}const b=e.target.closest('[data-action]');if(!b)return;const action=b.dataset.action;if(action==='review-issues'){app.outcomes=new Set(['refused','blocked','error']);app.mode='all';app.shown=PAGE_SIZE;syncControls();activateActivityView('events');activateTab('events');renderAll()}if(action==='stop'&&confirm('Stop the relay for every pod? Saved history will remain available.')){try{await mutate('stop');app.stopped=true;renderState();toast('Relay stopping')}catch(error){toast(error.message)}}if(action==='copy-url'){navigator.clipboard?.writeText(location.href);toast('Local URL copied')}if(action==='export'){location.href=apiBase+'/export'}if(action==='clear-history'&&confirm('Clear all retained relay activity? Pairing, sessions, and access rules will stay unchanged.')){try{await mutate('clear-history');toast('Local relay history cleared')}catch(error){toast(error.message)}}});
