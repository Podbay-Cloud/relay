...
`pb` ships to **two** places and both must move together, or the public source drifts behind
...
1. **npm** — `@podbay/pb`. Bump `version` in `package.json`, then `npm publish` from this dir.
2. **Public source mirror** — [`github.com/podbay-cloud/pb`](https://github.com/podbay-cloud/pb),
...
```bash
scripts/publish-pb-mirror.sh            # sync source → mirror, tag, cut the release
DRY_RUN=1 scripts/publish-pb-mirror.sh  # rehearse without pushing
```
